...
class Grid():
    ...
    # Clears out rolls (turns 'x's into '.'s). Returns whether or not any rolls were
    # cleared.
    def clear(self) -> bool:
...
with open(testfile, 'r') as file:
    ...
    while True:
        debug_print('\n')
        num_rolls_cleared += grid.transform()
        debug_print(grid)
        if not grid.clear():
            break

    print('\nTotal number of rolls cleared :', num_rolls_cleared)
